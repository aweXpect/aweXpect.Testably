...
partial class Build
{
	Target BenchmarkDotNet => _ => _
		.Executes(() =>
		{
			...
			DotNetBuild(s => s
				.SetProjectFile(Solution.Benchmarks.aweXpect_Testably_Benchmarks)
				.SetConfiguration(Configuration.Release)
				.EnableNoLogo());

			DotNet(
				$"{Solution.Benchmarks.aweXpect_Testably_Benchmarks.Name}.dll --exporters json --filter * --artifacts \"{benchmarkDirectory}\"",
				Solution.Benchmarks.aweXpect_Testably_Benchmarks.Directory / "bin" / "Release");
		});
	...
	Target BenchmarkResult => _ => _
		...
		.Executes(async () =>
		{
			string fileContent = await File.ReadAllTextAsync(ArtifactsDirectory / "Benchmarks" / "results" /
			                                                 "aweXpect.Testably.Benchmarks.HappyCaseBenchmarks-report-github.md");
		...
		.Executes(async () =>
		{
			...
			if (prId != null)
			{
				...
				IReadOnlyList<IssueComment> comments =
					await gitHubClient.Issue.Comment.GetAllForIssue("aweXpect", "aweXpect.Testably", prId.Value);
				long? commentId = null;
				...
				if (commentId == null)
				{
					...
					await gitHubClient.Issue.Comment.Create("aweXpect", "aweXpect.Testably", prId.Value, body);
				}
				else
				{
