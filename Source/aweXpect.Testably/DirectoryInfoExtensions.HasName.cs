...
{
	/// <summary>
	///     Verifies that the <see cref="IDirectoryInfo" /> has a name equal to <paramref name="expected" />.
	/// </summary>
	...
	private sealed class HasNameConstraint(
		...
		: ConstraintResult.WithValue<IDirectoryInfo>(grammars),
			IAsyncConstraint<IDirectoryInfo>
	{
		private string? _actualName;

		public async Task<ConstraintResult> IsMetBy(IDirectoryInfo actual, CancellationToken cancellationToken)
		{
			Actual = actual;
